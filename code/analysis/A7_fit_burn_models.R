...
burns_per_district <- 
  ...
  summarise( n_burns = sum( burned_binary, na.rm = T ) , 
             n_allots = n(), 
             area_km2 = sum(area)/1e6, 
             area_burned_km2 = sum(area_burned)/1e6 ) %>%
  mutate( log_area_km2 = log(area_km2) ) %>% 
  mutate( log_n_allots = log(n_allots)) %>% 
  mutate( area_burned_km2_int = as.integer(round(area_burned_km2 )))  %>% 
  arrange( ecogroup, district_label, year )

num_burns_form <- formula( n_burns ~ year2*ecogroup )

num_burns_form_mer_full <- 
  formula( n_burns ~ year2*ecogroup + (year2|district_label)) 

n_burns_poisson <- 
  glm( num_burns_form, 
       data = burns_per_district, 
       family = 'poisson', offset = log_n_allots)
...
burns_per_district$area_burned_10km2 <- burns_per_district$area_burned_km2/10

area_burned_mer <- 
  glmer(burn_area_form_mer_full, 
        data = burns_per_district, 
        offset = log_area_km2, 
        family = 'poisson', 
        control = control )

area_burn.nb <- glmer.nb(area_burned_10km2 ~ year2*ecogroup + (1|district_label), 
         data = burns_per_district, 
         offset = log_area_km2)

burns_per_district$area_predicted <- predict(area_burned_mer, type = 'response')

burns_per_district %>% 
  filter( ecogroup == 'E Cold Deserts') %>% 
  ggplot(aes( x = area_predicted, y = area_burned_km2_int)) + 
  geom_point() + 
  scale_x_log10() + 
  scale_y_log10()
...
burns_per_office_sample <- 
  burns_per_office %>% ungroup() %>% filter(row_number() <= 2000 )
...
burns_per_office$yhat <- predict( area_burn_glm )

burns_per_office %>%
  ungroup() %>% 
  filter( ecogroup == 'E Cold Deserts') %>%
  group_by( year ) %>%
  summarise( sum( exp(log_area_km2), na.rm = T), y_pred = sum(yhat)) %>%
  ggplot(aes( x = year, y = y_pred )) + geom_line() 
...
save(area_burn_glm, area_burn_mer, area_burn_mer2, burns_per_office, 
